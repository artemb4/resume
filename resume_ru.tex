...
Веду проекты со смежными сервисами, слежу за их стабильной работой.
...
% Yandex
    \resumeExpSubheading
      {\href{https://lavka.yandex.ru}{Яндекс Лавка\ExternalLink}}{Санкт-Петербург, Россия}
      {Backend-разработчик (С++) в Яндексе}{Октябрь 2022 --- Настоящее время}
      {\skill{C++} \skill{Python} \skill{PostgreSQL} \skill{boost} \skill{GTest} \skill{pytest}}
      \resumeDesc{
      \begin{itemize}
...
% Publear
    \resumeExpSubheading
      {\href{https://testflight.apple.com/join/XGv1ngxK}{Publear\ExternalLink}}{online}
      {Python-разработчик в проекте Publear}{Сентябрь 2020 --- Настоящее время}
      {\skill{Python} \skill{FastAPI} \skill{asyncio} \skill{Docker} \skill{Yandex Cloud} \skill{git}}
      \resumeDesc{
      \begin{itemize}
        \item С нуля добавил работу с Базами Данных PostgreSQL
        \item Добавил сервис для авторизации с google/yandex/email
        \item Добавил сервис для удаления данных
        \item Добавил i18n сервис
      \end{itemize}}

% IP
    \resumeExpSubheading
      {ИП Иванов}{Санкт-Петербург, Россия}
      {C\#-разработчик по разработке средства создания серверов для игры, схожей с Roblox}{Август 2021 --- Октябрь 2022}
      {\skill{C\#} \skill{sandbox} \skill{steam} \skill{git}}
      \resumeDesc{
      \begin{itemize}
...
\textbf{Frameworks:} & \quad Boost, Pytest, GTest, asyncio, FastAPI, Numpy \\
\textbf{Tools:} & \quad Git, Docker, Grafana, Elasticsearch, Charles
\end{tabular}
\resumeSubHeadingListEnd
